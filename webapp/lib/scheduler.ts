...
export type BubblerEvent = {
  job_id: string;
  kind: "run" | "link" | "code" | "test" | "restart";
  user_id: string;
  email: string;
  evony_name: string;
...
export async function claimNext(): Promise<BubblerEvent | null> {
  ...
  const row = (await d.get(
    ...
    [cutoff, nowIso()],
  )) as (Row & { job_id: string; kind: "run" | "link" | "code" | "test" | "restart"; user_id: string; email: string; evony_name: string; payload: string }) | undefined;
  if (!row) return null;
