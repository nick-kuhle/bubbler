...
## Recovering the agent from the War Room

The War Room agent card has a **Restart agent** button, operator-only (`nickkuhle@gmail.com`;
`is_operator` in the DB, not the agent bearer token). It exists because the agent is
outbound-only: Vercel cannot reach the laptop, so the button cannot run systemd itself.

What one click does:

1. `POST /api/admin/agent/restart` checks the operator session and the heartbeat, then
   enqueues a `restart` job in the same scheduler that delivers `run`/`link` events. A
   second click while one is pending is debounced.
2. The agent picks it up on its next long-poll (≤ ~46s). If a bubble run holds the device
   lock it is **deferred**, not cancelled — the run finishes and the restart is redelivered
   when the 90s lease expires.
3. The agent repairs the stack: checks the persistent phone key, SSH reachability, starts
   `frida-server` and the ZXTouch app if down, restarts the tunnel unit, and confirms
   `127.0.0.1:27042` and `127.0.0.1:6000`.
4. It POSTs each step to `/api/agent/maintenance` (bearer auth). The result shows on the
   War Room card as the last maintenance run, so you see which step failed.
5. The agent exits `0` and `bubbler-agent.service` (`Restart=always`) relaunches it with a
   new PID and a fresh heartbeat.

Clicking the button is the same repair as `bootstrap/admin_test.sh` without the local
output; use that script when you want the detail in a terminal.

**Limitation:** if the agent is fully dead it cannot consume the queued job, and the button
says so. Recovery then needs the phone awake and `systemctl --user start bubbler-agent`
(or `admin_test.sh`) on the laptop.

### Why reboots used to break this

The tunnel unit originally pointed at `/tmp/opencode/bubbler_phone_ed25519`, which macOS
wipes on reboot. The agent kept heartbeating, so the dashboard stayed green while every
touch silently failed, and `transport.py` fell back to a local `uiopen` that does not exist
in laptop mode. The key now lives in `~/.ssh/bubbler_phone_ed25519` (0700 dir, 0600 key),
regenerated by `ensure_phone_key.sh` and authorized on the phone. `transport.py` now fails
loudly instead of falling back, so this class of outage is visible rather than silent.

### Frida and ZXTouch are not the same kind of thing

Do not treat them alike, because only one of them needs our help:

- **frida-server self-heals.** The jailbreak already runs it as a launchd KeepAlive job
  (`/var/jb/Library/LaunchDaemons/re.frida.server.plist`, `RunAtLoad`, `UserName root`,
  `ThrottleInterval 5`). Kill it and it is back in about a second. `repair.py` therefore
  *waits* for launchd before doing anything — force-starting frida underneath a running
  launchd job races for port 27042 and can leave two owners flapping.
- **ZXTouch does not.** It is an app, not a daemon, so nothing restarts it after a reboot
  or a respring. It must be re-launched by hand, which is why `repair.py` treats a missing
  ZXTouch as a repairable fault.

If the frida launchd job is ever removed or disabled, `repair.py` can start frida itself,
but only as **root** — it ptraces the game, and the jailbreak's `sudo` asks for a password,
so the only unattended route is key-based root SSH. Set `phone.root_ssh_key` and add the
laptop's public key to the phone's root `authorized_keys`. On Dopamine that path is
`/var/jb/var/root/.ssh/authorized_keys`, **not** `/var/root` — `getent passwd root` reports
the `/var/jb` one, and a key in the wrong directory fails with a bare
"Permission denied (publickey)". With no root key configured the repair degrades to saying
frida needs a manual start instead of failing obscurely.